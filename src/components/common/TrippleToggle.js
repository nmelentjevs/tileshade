...
export default class TrippleToggle extends Component {
  constructor(props) {
    super(props);
  ...
  _onPanResponderMove = (evt, gestureState) => {
    if (
      [this.MIN_VALUE, this.MID_VALUE, this.MAX_VALUE].includes(
        this._switchvalue
      )
    ) {
    ...
    if (this._switchvalue >= this.MAX_VALUE) {
      ...
      this._animatedValue.setValue(this.animatedSetValue);
      if (
        this.animatedSetValue < this.MID_LEFT_VALUE &&
        this.state.calledState !== 'left'
      ) {
        this.onLeftState((animate = false));
      } else if (
        this.animatedSetValue > this.MID_LEFT_VALUE &&
        this.animatedSetValue < this.MID_RIGHT_VALUE &&
        this.state.calledState !== 'middle'
      ) {
  ...
  _onPanResponderRelease = (evt, gestureState) => {
    if (this._switchvalue >= this.MID_RIGHT_VALUE) {
      ...
      this._switchvalue >= this.MID_LEFT_VALUE &&
      this._switchvalue <= this.MID_RIGHT_VALUE
    ) {
      this._animateToSwitchItem(this.MID_VALUE);
    } else {
      this._animateToSwitchItem(this.MIN_VALUE);
  ...
  renderToggleItems = () => {
    ...
    const AnimatedJelly = this.props.AnimatedJelly;
    const AnimatedCat = this.props.AnimatedCat;
    const AnimatedEagle = this.props.AnimatedEagle;
    ...
    return (
      <View style={[this.props.itemsContainer]}>
        ...
        >
          <AnimatedJelly
            size={35}
            fill={this._animatedValue.interpolate({
        ...
        <TouchableOpacity
          style={[this.props.itemContainer, { left: -4, elevation: 100 }]}
